...
- **Manual FiberFailure unwrapping in `registerCompileCommand`.** [main.ts:288](packages/extension/src/extension/main.ts#L288), [main.ts:299](packages/extension/src/extension/main.ts#L299). Use `runPromiseExit`/`Cause` (pairs with B7/B42). *(Open — deferred to the `extension/main.ts` god-module decomposition, since the cleanup is most natural while extracting the compile command.)*
