...
For every file-path tool **except `Bash`** (`Read`, `Edit`, `Glob`, `Grep`, Serena tools), use Windows backslash paths and expand `~` to the full home dir.
...
- **Effect LSP**: `@effect/language-service` plugin is in the root tsconfig (editor diagnostics only; tsgo build ignores it). CLI: `pnpm exec effect-language-service diagnostics --project <tsconfig>`.
...
Serena MCP (LSP-backed code intelligence) is configured. For **code files**, prefer Serena over `grep`/`Read`/`Edit` — token-efficient and structure-aware. Detailed file/symbol inventories are intentionally NOT duplicated in this doc; discover them with Serena.
...
## DSL features (one-liners; details in `specs/<NNN>/` and via Serena)
