 ...
 * Locale File Watcher
 *
 * Watches locale JSON files for external changes and triggers reload
 ...
 *
 * Design:
 * - Single FileSystemWatcher for each locale file being edited
 * - Debouncing (300ms) to handle auto-save
 * - Graceful cleanup on disposal
 */
export class LocaleFileWatcher implements vscode.Disposable {
  private watcher: vscode.FileSystemWatcher | null = null;
  private debounceTimer: NodeJS.Timeout | null = null;
  private readonly debounceDelay = 300; // milliseconds
  private readonly onChange: LocaleFileChangeCallback;
  private readonly fileUri: vscode.Uri;

  /**
   * Create a new locale file watcher
   *
   * @param fileUri - URI of the locale file to watch
   * @param onChange - Callback invoked when file changes (after debouncing)
   */
  constructor(fileUri: vscode.Uri, onChange: LocaleFileChangeCallback) {
  ...
  /**
   * Start watching the locale file
   */
  private startWatching(): void {
    ...
    this.watcher.onDidDelete(uri => {
      if (uri.toString() === this.fileUri.toString()) {
        ...
        vscode.window.showWarningMessage(`Locale file '${uri.fsPath}' was deleted.`);
      }
    });
  }
