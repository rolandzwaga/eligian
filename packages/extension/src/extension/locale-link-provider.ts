/**
 * LocaleLinkProvider.ts - Clickable links for locale imports
 *
 * Purpose: Make locale import paths clickable to open in Locale Editor.
...
/**
 * Provides clickable links for locale import statements.
 *
 * When the user Ctrl+Clicks on a locale import path, this provider:
 * 1. Detects the locale import line
 * 2. Creates a clickable range for the path
 * 3. Generates a command URI to open the file in Locale Editor
 *
 * @example
 * const provider = new LocaleLinkProvider();
 ...
 * );
 */
export class LocaleLinkProvider implements vscode.DocumentLinkProvider {
  /**
   * Provide document links for locale imports.
   *
  ...
  public provideDocumentLinks(
    ...
    for (let lineNum = 0; lineNum < document.lineCount; lineNum++) {
      ...
      while ((match = pattern.exec(text)) !== null) {
        ...
        if (fileUri) {
          // Create a command URI to open with Locale Editor
          // Uses our custom command registered in main.ts
          const commandUri = vscode.Uri.parse(
            `command:eligian.openLocaleFile?${encodeURIComponent(JSON.stringify([fileUri.toString()]))}`
          );

          const link = new vscode.DocumentLink(range, commandUri);
          link.tooltip = 'Open in Locale Editor';
          links.push(link);
        }
      }
    }
