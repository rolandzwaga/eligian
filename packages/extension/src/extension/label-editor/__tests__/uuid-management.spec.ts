/**
 * Label Editor UUID Management Tests (Feature 036, User Story 3)
 *
 ...
 *
 * These tests verify the UUID handling logic in LabelValidation.ts
 * and the LabelEditorProvider's parseLabels method.
 */
...
import { generateUUID, validateUUID } from '../LabelValidation.js';
...
describe('Label Editor UUID Management (Feature 036, User Story 3)', () => {
  ...
  describe('T034: Auto-fix missing or invalid UUIDs on document load', () => {
    /**
     * Simulates the parseLabels logic from LabelEditorProvider
  ...
  describe('T035: Never display UUID values in the UI', () => {
    it('should not render UUID in translation card (verified in webview script)', () => {
      ...
      const scriptPath = path.join(__dirname, '..', '..', '..', '..', 'media', 'label-editor.ts');
    ...
    it('should not show UUID in HTML template (verified in template)', () => {
      const templatePath = path.join(__dirname, '..', 'templates', 'label-editor.html');
