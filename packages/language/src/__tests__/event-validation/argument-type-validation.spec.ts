...
describe('Argument Type Compatibility Validation (US3)', () => {
  ...
  test('should accept matching type for single parameter', async () => {
    const code = `
      ...
      on event "dom-mutation" action HandleMutation(payload: any) [
        selectElement("#app")
      ]
  ...
  test('should warn about unnecessary type annotation for extra parameter', async () => {
    const code = `
      ...
      on event "timeline-complete" action HandleComplete(extraParam: string) [
        selectElement("#app")
      ]
  ...
  test('should skip type validation for unknown events', async () => {
    const code = `
      ...
      on event "unknown-event" action HandleUnknown(a: string, b: number) [
        selectElement("#app")
      ]
  ...
  test('should enforce case-sensitive type matching', async () => {
    const code = `
      ...
      on event "dom-mutation" action HandleMutation(payload: string) [
        selectElement("#app")
      ]
